...
class Command(BaseCommand):
    help = "Fetch businesses with phones from Yelp Fusion and save PhoneRecords."

    def add_arguments(self, parser):
        parser.add_argument("--city", required=True, help="US city, e.g. Austin")
        parser.add_argument(
            "--category",
            default="restaurants",
            help="Yelp category alias, e.g. restaurants, dentists, plumbers",
        )
        parser.add_argument("--limit", type=int, default=25)

    def handle(self, *args, **options):
        self.stdout.write(
            f"Searching Yelp: {options['category']} in {options['city']}..."
        )
        try:
